...
# output: unfolding size x hidden size, state = hideen size
output, _states = tf.nn.dynamic_rnn(multi_cells, X_one_hot, dtype=tf.float32)

# FC layer
X_for_fc = tf.reshape(output, [-1, hidden_size])
output = tf.contrib.layers.fully_connected(X_for_fc, num_classes, activation_fn=None)
...
for i in range(500):
    _, I, results = sess.run([train_op, mean_loss, output],
