...
W2 = tf.get_variable("W2", shape=[512, 512], initializer=tf.contrib.layers.xavier_initalizer())
b2 = tf.Variable(tf.random_normal([512]))
L2 = tf.nn.relu(tf.matmul(L1, W2) + b2)

W3 = tf.get_variable("W3", shape=[512, 512], initializer=tf.contrib.layers.xavier_initalizer())
b3 = tf.Variable(tf.random_normal([512]))
L3 = tf.nn.relu(tf.matmul(L2, W3) + b3)

W4 = tf.get_variable("W4", shape=[512, 512], initializer=tf.contrib.layers.xavier_initalizer())
b4 = tf.Variable(tf.random_normal([512]))
hypothesis = tf.matmul(L3, W4) + b4

cost = tf.random_normal(tf.nn.softmax_cross_entropy_with_logits(logits=hypothesis, labels=Y))
